...
func (c *App) getAccessToken() (tokenInfo, error) {
	log.Sugar.Info("fetch access_token")
	...
	for {
		...
		case <-time.After(waitDuration):
			...
			if err := backoff.Retry(t.syncToken, retryer); err != nil {
				log.Sugar.Errorw("retry getting access toke failed", "err", err)
				_ = err
			}

			waitDuration = t.lastRefresh.Add(t.expiresIn).Add(-refreshTimeWindow).Sub(t.lastRefresh)
			log.Sugar.Infow("access_token", "token", t.token, "expiresIn", t.expiresIn, "nextRefreshTime", waitDuration)
