...
type ChatMsg struct {
	...
	// 聊天的文本内容
	ContentText string `gorm:"type:varchar(512);class:FULLTEXT,option:WITH PARSER ngram INVISIBLE" json:"content_text"`
...
type ChatSessions struct {
	...
	// 会话对方的客户类型 1-微信 2-企业微信，仅外部客户时存在
...
func (o ChatMsg) QueryMsg(
	req requests.QueryChatMsgReq, extCorpID string, sendAtStart, sendAtEnd *time.Time, sorter *app.Sorter, pager *app.Pager) (msgs []ChatMessage, total int64, err error) {
	...
	db := DB.Table("chat_msg").
		...
		Where("chat_msg.ext_corp_id =?", extCorpID)

	if req.ReceiverID != "" && req.ExtStaffID != "" {
		db = db.Where("( ( json_contains(to_list, json_array(?)) and `from` = ? ) or ( json_contains(to_list, json_array(?)) and `from` = ? ))",
			req.ReceiverID, req.ExtStaffID, req.ExtStaffID, req.ReceiverID)
		//Or("json_contains(to_list, json_array(?)) and `from` = ? ", req.ExtStaffID, req.ReceiverID)
	}

	if req.MsgType != "" {
		db = db.Where("msg_type = ?", req.MsgType)
	}

	if req.MaxID != "" || req.MinID != "" {
		pager.SetDefault()
		db = db.Offset(pager.GetOffset()).Limit(pager.GetLimit())
		// 查上下文
		if req.MaxID != "" {
			db.Where("chat_msg.id > ?", req.MaxID).
				Order(clause.OrderByColumn{Column: clause.Column{Name: "chat_msg.id"}, Desc: sorter.SortType == constants.SortTypeDesc})
		} else if req.MinID != "" {
			db.Where("chat_msg.id < ?", req.MinID).
				Order(clause.OrderByColumn{Column: clause.Column{Name: "chat_msg.id"}, Desc: sorter.SortType == constants.SortTypeDesc})
		}
		// ChatMessage 会话中的消息列表条目
		...
		// 按时间范围查询
		log.Sugar.Debugw("start at ", "time", sendAtStart)
...
func (o ChatMsg) QuerySessions(
	extStaffID string, sessionType string, extCorpID string, name string, pager *app.Pager) (chatSessions []ChatSessions, total int64, err error) {

	if name != "" {
		var rawSql string
		if sessionType == string(constants.ChatSessionTypeInternal) {
			rawSql = "select * from ( " +
				" select chat_msg.*, " +
				" rank() over (partition by s.name order by chat_msg.msg_time) r, s.name as peer_name, s.avatar_url as peer_avatar, s.type as peer_type, s.ext_id as peer_ext_id " +
				" 	from chat_msg left join staff s on chat_msg.`from` = s.ext_id or json_contains(chat_msg.to_list, json_array(s.ext_id)) where s.name like ? " +
				" ) a where a.r = 1;"
		} else if sessionType == string(constants.ChatSessionTypeExternal) {
			rawSql = " select * from ( " +
				" select chat_msg.*," +
				" 	rank() over (partition by c.name order by chat_msg.msg_time) r, c.name  as peer_name, c.avatar  as peer_avatar, c.type as peer_type, c.ext_id  as peer_ext_id " +
				" 	from chat_msg left join customer c on chat_msg.`from` = c.ext_id or json_contains(chat_msg.to_list, json_array(c.ext_id)) where c.name like ? " +
				" ) a where a.r = 1; "
		} else if sessionType == string(constants.ChatSessionTypeGroup) {

		}
		err = DB.Raw(rawSql, name+"%").Find(&chatSessions).Error
		if err != nil {
	...
	case constants.ChatSessionTypeExternal:
		...
		for i, item := range chatSessions {
			...
			if item.From == extStaffID {
				...
				chatSessions[i].PeerType = avatarMap[item.ToList[0]].Type
				...
				chatSessions[i].PeerType = avatarMap[item.From].Type
			}
		}
	case constants.ChatSessionTypeInternal:
		err = DB.Model(&Staff{}).
			Where("ext_id in (?)", extIDs).Select("name, ext_id,avatar_url as avatar").Find(&avatars).Error
		if err != nil {
			err = errors.WithStack(err)
			return
		}
...
func (o ChatMsg) SearchMsg(chatMsg ChatMsg, pager *app.Pager) (msgs []ChatMessage, total int64, err error) {
	db := DB.Table("chat_msg").
		Joins("left join customer c on c.ext_id = chat_msg.ext_creator_id").
		Joins("left join staff s on s.ext_id = chat_msg.ext_creator_id").
		Where("chat_msg.ext_corp_id = ?", chatMsg.ExtCorpID).
		Where("match (content_text) against (?)", chatMsg.ContentText).
		Where("json_contains(to_list, json_array(?)) and `from` = ? ", chatMsg.ToList[0], chatMsg.From).
		Or("json_contains(to_list, json_array(?)) and `from` = ? ", chatMsg.From, chatMsg.ToList[0])
